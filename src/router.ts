...
export class Router {
    ...
    private readonly _rules: Array<Rule> = [{
        ...
        handler: async (request, env, match) => await this.onMavenRepo(request, env, Type.github, 'echocat', 'hugo-maven-plugin', match[1]),
    }, {
        regexp: /^\/maven\/(org\/echocat\/units4j\/units4j\/.+)$/,
        handler: async (request, env, match) => await this.onMavenRepo(request, env, Type.github, 'echocat', 'units4j', match[1]),
    }, {
        regexp: /^\/maven\/(org\/echocat\/locela\/api\/java\/.+)$/,
        handler: async (request, env, match) => await this.onMavenRepo(request, env, Type.github, 'echocat', 'locela-api-java', match[1]),
    }, {
        regexp: /^\/maven\/(org\/echocat\/java-stream-utils\/java-stream-utils\/.+)$/,
        handler: async (request, env, match) => await this.onMavenRepo(request, env, Type.github, 'echocat', 'java-stream-utils', match[1]),
    }, {
        regexp: /^\/maven\/(org\/echocat\/repo4j\/repo4j\/.+)$/,
        handler: async (request, env, match) => await this.onMavenRepo(request, env, Type.github, 'echocat', 'repo4j', match[1]),
    }, {
    ...
    public onNotFound: (request: Request, env: Environment, headers?: HeadersInit) => Promise<Response> = async (request, _, headers = {}) => this._defaultOnNotFound(request, headers);

    public onMavenRepo: (request: Request, env: Environment, type: Type, organization: Organization, repository: Repository, file: string) => Promise<Response> = async (request, env, type, organization, repository, file) => {
    ...
    public onHelmIndex: (request: Request, env: Environment) => Promise<Response> = async (request, env) => {
        const target = new URL(request.url);
        target.pathname = '/helm/index.yaml';
        return Response.redirect(target.toString(), 301);
    }

    public onIndex: (request: Request, env: Environment) => Promise<Response> = async () => Response.redirect('https://github.com/echocat/packages.echocat.org', 301);
    ...
    private async _respondWithError(_: Request, statusCode: number, status: string, message: string, headers: HeadersInit = {}) {
