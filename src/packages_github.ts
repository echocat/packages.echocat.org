   ...
   private async _findMavenFile(request: Request, env: Environment, repository: Repository, file: string) {
      ...
      for (let run = 0; run < 10; run++) {
         ...
         if (fetchedResponse.status >= 400 && fetchedResponse.status !== 404) {
            ...
            console.error(
               `Cache missed, need to retrieve it: ${this.organization}/${repository}/${file}... FAILED (reason: ${fetchedResponse.status} - ${fetchedResponse.statusText})!`,
            );
         ...
         toCacheResponse.headers.set(
            'Cache-Control',
            `public, max-age=${fetchedResponse.status >= 400 ? ttlNotFound : ttl}, public`,
         );
         await cache.put(cacheKey, toCacheResponse);

         console.info(
            `Cache missed, need to retrieve it: ${this.organization}/${repository}/${file}... DONE (exists: ${fetchedResponse.status < 400})!`,
         );
      }

      console.error(
         `Cache missed, need to retrieve it: ${this.organization}/${repository}/${file}... FAILED (reason: unknown)!`,
      );
                     ...
                     for (const asset of release.assets) {
                        if (asset.name) {
                           ...
                           if (releaseMatch && asset.browser_download_url) {
                              ...
                              console.debug(
                                 `Found github.com/${this.organization}/${repo}/${release.name}/${asset.name}`,
                              );
                              ...
                              for (const subAsset of release.assets) {
                                 if (subAsset.name === `${asset.name}.sha256` && subAsset.browser_download_url) {
                                    ...
                                    console.debug(
                                       `Found sha256 github.com/${this.organization}/${repo}/${release.name}/${subAsset.name}`,
                                    );
                                 }
                                 if (subAsset.name === `${prefix}-helm-chart.yaml` && subAsset.browser_download_url) {
                                    ...
                                    console.debug(
                                       `Found chart github.com/${this.organization}/${repo}/${release.name}/${subAsset.name}`,
                                    );
                                 }
                              }
